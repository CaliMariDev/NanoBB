...
```
Next you can compile code by making a new instance. It automatically compiles the string you pass into the constructor. You can also recompile with the "compile" function. You also can pass an "io" parameter which is just a function which can be accessed and called by the code via the "io_req" function. (only exists if the code is directly called via ".exec()" otherwise, you need to pass the function directly into the executable)
```js
try {
